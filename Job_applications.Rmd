...
Here we generated a bar plot to visualize missing values in a dataset, using custom colors and a minimalistic theme.
...
plot <- ggplot(missing_data, aes(x = Variable, y = Missing_Values)) +
  geom_bar(stat = "identity", fill = box_color) +
...
Here we generated a bar plot displaying counts for specific statistics, added custom colors, included a percentage table, and arranged both the plot and table using the `grid.arrange` function.

```{r Basic Statistics Plots}

stat_labels <- c("Rows", "All missing columns", "Missing observations", "Complete Rows", "Total observations")
statistics <- c(100, 20, 30, 40, 200)
