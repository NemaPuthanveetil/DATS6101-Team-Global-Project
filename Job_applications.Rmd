...
employfit <- rpart(Employed ~ ., data=df_final2)# instead of employfit, dt_model
...
p1 <- predict(pemployfit, test, type = 'prob')# instead of p1 dt_pred
p1 <- p1[,2]
r <- roc(test$Employed, p1, percent = TRUE)# roc_dt
auc(r)
plot(r)
```
...
plot(roc_lr3, print.auc = T, main = "ROC Curve for LR Model (With 3 variables)", col = "#514BA2", lwd = 2)
```
