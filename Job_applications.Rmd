...
SOME INSTRUCTIONS FOR NOW:\
Standardization is not a necessity. Experiment with standardized and non standardized data in model building. There is also another way of feature scaling called normalization, which scales the data into the range 0 and 1. In general, normalization works better for KNN(since this doesn't assume normal distribution), and, standardization works better for PCA. For Tree algorithms, scaling may hinder performance.
...
# x_train = subset(train, select = -c(Employed))
# x_test = subset(test, select = -c(Employed))
# y_train = subset(train, select = c(Employed))
# y_test = subset(test, select = c(Employed))
# 
# head(x_train)
# head(x_test)
# head(y_train)
...
For our work, I'm dividing this into 4 sections where each of us will experiment with different models
...
```
Let's do a quick class imbalance check before deciding on the evaluation metric.

``` {r Imbalance check}
ggplot(data = df_final, aes(Employed, after_stat(count))) + 
  geom_bar(alpha = 0.5)

```

Since there's no such major imbalance, and we want to know the overall performance of the model, we'll use AUC ROC as our evaluation metric

```{r AUC ROC Evaluation}
...
accuracy <- sum(knn_pred == test$Employed) / nrow(test)
print(accuracy)

```

```{r}
# Assuming numeric features
train_scaled <- scale(train[, sapply(train, is.numeric)])
train_scaled$Employed <- train$Employed  # Adding the target variable back
svm_model <- svm(Employed ~ ., data = train_scaled, kernel = "linear", probability = TRUE)
...
## Nema
```{r, Growing and Plotting the Decision tree}
# Growing the tree
set.seed(1)
# employfit <- rpart(Employed ~ Age + Accessibility + EdLevel + Employment + MentalHealth + MainBranch + YearsCode + YearsCodePro + PreviousSalary + Gender_Man + Gender_NonBinary + Gender_Woman, data=train)
employfit <- rpart(Employed ~ ., data=df_final2)
...
# plot tree 
loadPkg("rpart.plot")
rpart.plot(employfit, main="Decision Tree for Employment")
...
#prune the tree 
pemployfit <- prune(employfit, cp = employfit$cptable[2,"CP"])
#pemployfit <- prune(employfit, cp = employfit$cptable[which.min( employfit$cptable[,"xerror"] ),"CP"])


rpart.plot(pemployfit, main="Pruned Decision Tree for Employment")
```

```{r, Decision Tree ROC and AUC}
p1 <- predict(pemployfit, test, type = 'prob')
p1 <- p1[,2]
...
lr_model_test <- glm(Employed ~ Age + Accessibility + EdLevel + Employment + MentalHealth + MainBranch + YearsCode + YearsCodePro + PreviousSalary + Gender_Man + Gender_NonBinary + Gender_Woman, data = train, family = "binomial")
summary(lr_model_test)

pred=predict(lr_model_test, test, type = "response" )
# test$prob=prob
roc_lr_test <- roc(test$Employed, pred)
auc(roc_lr_test) # area-under-curve prefer 0.8 or higher.
plot(roc_lr_test)
...
# rf <- randomForest(Employed ~., data=train, proximity=TRUE) 
# print(rf)
...
# 
# rf <- randomForest(Employed ~ ., data = train, proximity = TRUE, ntree = 100, mtry = 3)
# 
# stopCluster(cl)  # Stop the parallel processing cluster

# rf <- randomForest(Employed ~ ., data = train, proximity = TRUE, ntree = 50, mtry = 2)
...
```{r, q3 Decision Tree}
set.seed(1)
employfit3 <- rpart(Employed ~ Age +  EdLevel + ComputerSkills, data=train)

printcp(employfit3) # display the results 
plotcp(employfit3) # visualize cross-validation results 
summary(employfit3) # detailed summary of splits
printcp(employfit3)

# plot tree 
loadPkg("rpart.plot")
rpart.plot(employfit3, main="Decision Tree for Employment")

p1 <- predict(employfit3, test, type = 'prob')
p1 <- p1[,2]
r <- roc(test$Employed, p1, percent = TRUE)
auc(r)
plot(r)
...
for (minsplit_val in minsplit_vals) {
  for (max_depth_val in max_depth_vals) {
    # employfit_loop <- rpart(Employed ~ Age + Accessibility + EdLevel + Employment + MentalHealth + MainBranch + YearsCode + YearsCodePro + PreviousSalary + Gender_Man + Gender_NonBinary + Gender_Woman, data=train, minsplit = minsplit_val, maxdepth = max_depth_val)
    employfit_loop <- rpart(Employed ~ ., data=train, minsplit = minsplit_val, maxdepth = max_depth_val)
    p1_loop <- predict(employfit_loop, test, type = 'prob')
    p1_loop <- p1_loop[,2]
